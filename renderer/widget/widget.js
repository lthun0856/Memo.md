...
const els = {
  ...
  btnHandleOnly: document.getElementById('btnHandleOnly'),
...
// ---- 메모연결(링크) 표시 점: 눈 아이콘 옆에 작은 점으로 "링크함/링크받음" 표시 ----
// 링크는 항상 내보낸 파일명([[파일명]])을 가리키므로, "링크받음"은 이 메모가 내보낸 적
// 있어야만 의미가 있음(내보낸 적 없으면 그 메모를 가리킬 방법 자체가 없어서 항상 false)
function memoLinkKey(m) {
  if (!m.obsidian || !m.obsidian.filePath) return null;
  const base = m.obsidian.filePath.split(/[\\/]/).pop() || '';
  return base.replace(/\.md$/i, '');
}
function hasOutgoingLink(m) {
  return /\[\[.+?\]\]/.test(m.content || '');
}
function hasIncomingLink(m) {
  const key = memoLinkKey(m);
  if (!key) return false;
  const needle = `[[${key}]]`;
  return memos.some((other) => other.id !== m.id && (other.content || '').includes(needle));
}
...
function renderDashboard() {
  ...
  visibleTopics().forEach((t) => {
    ...
    list.forEach((m) => {
      ...
      // 링크 표시 점 2개(있을 때만 보임): 이 메모가 다른 메모를 링크함 / 다른 메모가 이 메모를 링크함
      if (hasOutgoingLink(m)) {
        const outDot = document.createElement('span');
        outDot.className = 'memo-link-dot';
        outDot.title = LANG.widget.memoLinkOutgoing;
        item.appendChild(outDot);
      }
      if (hasIncomingLink(m)) {
        const inDot = document.createElement('span');
        inDot.className = 'memo-link-dot';
        inDot.title = LANG.widget.memoLinkIncoming;
        item.appendChild(inDot);
      }
...
function syncCollapsedWidth() {
  ...
  const handleOnlyBtnWidth = els.btnHandleOnly ? els.btnHandleOnly.offsetWidth : 0;

  // 완전축소(handleOnly) 모드면 주제버튼/다른 버튼 계산은 다 무시하고, 토글버튼+손잡이만
  // 딱 맞게 들어가는 폭으로 줄임(위 WIDGET_COLLAPSED_WIDTH_MIN 최소폭도 적용 안 함).
  // 보이는 조각이 2개(토글버튼/손잡이)라 사이 gap은 1번만 들어감
  if (settings.widget.handleOnly) {
    const width = Math.ceil(titlebarPadding + handleOnlyBtnWidth + dragHandleWidth + titlebarGap + WIDGET_COLLAPSED_WIDTH_BUFFER);
  ...
  // 타이틀바 안에 보이는 조각 5개(완전축소 토글버튼 / 손잡이 / 주제버튼들 / 설정 관련 아이콘 /
  // 눈 아이콘) 사이에 gap이 4번 들어감
  const desired = Math.ceil(
    titlebarPadding + handleOnlyBtnWidth + dragHandleWidth + iconsWidth + actionsWidth + alwaysActionsWidth
    + titlebarGap * 4 + WIDGET_COLLAPSED_WIDTH_BUFFER
  );
...
// 완전축소 토글 버튼: 손잡이 왼쪽의 작은 버튼. 1단계 접힘(정사각형)에서 누르면 주제버튼/
// 다른 버튼을 다 숨기고 손잡이만 남는 2단계(가로 좁은 직사각형)로, 그 상태에서 다시 누르면
// 원래 1단계로 돌아옴. 더블클릭을 손잡이(드래그 영역) 자체에서 판정하려 했던 이전 방식은
// OS가 그 드래그 영역의 클릭 계열 이벤트를 가로채서 안 먹는 문제가 있었음(위 .collapsed-icons
// 주석 참고 — 이미 한 번 겪었던 문제라 JS로는 못 고침). 그래서 드래그 영역이 아닌 일반
// 버튼으로 분리해서 평범한 한 번 클릭으로 바꿈
els.btnHandleOnly.addEventListener('click', async () => {
  const nextValue = !settings.widget.handleOnly;
  const widgetSettings = await window.api.setWidgetHandleOnly(nextValue);
  settings.widget = widgetSettings;
  applyWidgetState();
  requestAnimationFrame(syncCollapsedWidth);
});
